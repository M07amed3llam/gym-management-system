...
namespace SeVenGYM2.Project.Forms
{
    public partial class SalesReport : Form
    {
        ...
        public void SumPrice()
        {
            ...
            {
                sum += Convert.ToInt32(dgvSalesReport.Rows[i].Cells[3].Value);
            }
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                using (SqlConnection con = new SqlConnection(CS))
                {
                    SqlCommand cmd = new SqlCommand("spSalesReport", con);
                    ...
                    Da.Fill(Dt);
                    this.dgvSalesReport.DataSource = Dt;
                    lblTotalSessions.Text = dgvSalesReport.Rows.Count.ToString();
                    SumPrice();
                }
            }
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvSalesReport.Rows.Count > 0)
                {
                    ...
                    {
                        XcelApp.Cells[1, i] = dgvSalesReport.Columns[i - 1].HeaderText;
                    }
                    for (int i = 0; i < dgvSalesReport.Rows.Count; i++)
                    {
                        for (int j = 0; j < dgvSalesReport.Columns.Count; j++)
                        {
                            XcelApp.Cells[i + 2, j + 1] = dgvSalesReport.Rows[i].Cells[j].Value.ToString();
                        }
                    }
        ...
        private void SalesReport_Load(object sender, EventArgs e)
        {
            ...
            dgvSalesReport.ColumnHeadersDefaultCellStyle.BackColor = Color.Gray;
