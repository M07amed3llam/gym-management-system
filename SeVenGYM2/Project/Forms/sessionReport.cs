...
namespace SeVenGYM2.Project.Forms
{
    public partial class sessionReport : Form
    {
        ...
        {
            int sum = 0;
            for (int i = 0; i < dgvSessionReport.Rows.Count; ++i)
            {
                sum += Convert.ToInt32(dgvSessionReport.Rows[i].Cells[3].Value);
            }
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("spSessionReport", con);
                    ...
                    Da.Fill(Dt);
                    this.dgvSessionReport.DataSource = Dt;
                    lblTotalSessions.Text = dgvSessionReport.Rows.Count.ToString();
                    SumPrice();
                }
            }
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvSessionReport.Rows.Count > 0)
                {
                    Microsoft.Office.Interop.Excel.Application XcelApp = new Microsoft.Office.Interop.Excel.Application();
                    XcelApp.Application.Workbooks.Add(Type.Missing);
                    for (int i = 1; i < dgvSessionReport.Columns.Count + 1; i++)
                    {
                        XcelApp.Cells[1, i] = dgvSessionReport.Columns[i - 1].HeaderText;
                    }
                    for (int i = 0; i < dgvSessionReport.Rows.Count; i++)
                    {
                        for (int j = 0; j < dgvSessionReport.Columns.Count; j++)
                        {
                            XcelApp.Cells[i + 2, j + 1] = dgvSessionReport.Rows[i].Cells[j].Value.ToString();
                        }
                    }
        ...
        {
            dgvSessionReport.EnableHeadersVisualStyles = false;
            dgvSessionReport.ColumnHeadersDefaultCellStyle.BackColor = Color.Gray;
            dgvSessionReport.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
        }
    }
}
