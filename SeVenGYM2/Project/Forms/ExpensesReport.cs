...
namespace SeVenGYM2.Project.Forms
{
    public partial class ExpensesReport : Form
    {
        ...
        {
            int sum = 0;
            for (int i = 0; i < dgvEXReport.Rows.Count; ++i)
            {
                sum += Convert.ToInt32(dgvEXReport.Rows[i].Cells[2].Value);
            }
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    SqlCommand cmd = new SqlCommand("spExpenesReport", con);
                    ...
                    Da.Fill(Dt);
                    this.dgvEXReport.DataSource = Dt;
                    lblTotalSessions.Text = dgvEXReport.Rows.Count.ToString();
                    SumPrice();
                }
            }
        }
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvEXReport.Rows.Count > 0)
                {
                    Microsoft.Office.Interop.Excel.Application XcelApp = new Microsoft.Office.Interop.Excel.Application();
                    XcelApp.Application.Workbooks.Add(Type.Missing);
                    for (int i = 1; i < dgvEXReport.Columns.Count + 1; i++)
                    {
                        XcelApp.Cells[1, i] = dgvEXReport.Columns[i - 1].HeaderText;
                    }
                    for (int i = 0; i < dgvEXReport.Rows.Count; i++)
                    {
                        for (int j = 0; j < dgvEXReport.Columns.Count; j++)
                        {
                            XcelApp.Cells[i + 2, j + 1] = dgvEXReport.Rows[i].Cells[j].Value.ToString();
                        }
                    }
        ...
        {
            dgvEXReport.EnableHeadersVisualStyles = false;
            dgvEXReport.ColumnHeadersDefaultCellStyle.BackColor = Color.Gray;
            dgvEXReport.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
        }
    }
}
